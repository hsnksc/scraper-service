...
class HepsieEmlakScraper(BaseScraper):
    ...
    async def extract(self, page: Page) -> Optional[dict]:
        ...
        description = await _safe_text(page, ".description-text, .detail-description, p.description") or ""
        body_text = await self._get_body_text(page)

        # Price - look for price-related elements
        price_raw = await _safe_text(page, '[class*="price"], [class*="fiyat"]') or ""
        if not price_raw:
            # Try finding any element with TL, ₺
            for line in body_text.split("\n"):
                if any(sym in line for sym in ["₺", "TL", "Bin"]):
                    price_raw = line
                    break
        price = extract_price(price_raw) or extract_first_price(body_text) or extract_price(title)

        # Details table - hepsiemlak uses detayTable
        details = await self._extract_details(page, body_text=body_text, title=title)

        # Images
        images = await _safe_all_attrs(page, ".detail-gallery img, .gallery img, [class*='gallery'] img", "src")
        images = [img for img in images if img and len(img) > 30 and "logo" not in img.lower()]

        # Location from breadcrumbs or title
        loc = await _safe_text(page, ".location-breadcrumb, [class*='location'], .address-info") or ""
        if not loc:
    ...
    async def _extract_details(self, page: Page, body_text: str = "", title: str = "") -> dict:
        result = {}
        try:
            # HepsieMlak uses detail rows with label/value
            detail_rows = page.locator("[class*='detail'] li, .detail-table li, [class*='features'] li")
            count = await detail_rows.count()
            for i in range(count):
                text = await detail_rows.nth(i).inner_text(timeout=2000)
                text = text.strip()
                if not text:
                    continue
                if any(k in text.lower() for k in ["m²", "metrekare", "net m", "brut m"]):
                    area = extract_area(text)
                    if area:
                        result["area_sqm"] = area
                elif any(k in text.lower() for k in ["oda", "oda say"]):
                    rooms = extract_rooms(text)
                    if rooms:
                        result["rooms"] = rooms
                elif "daire" in text.lower() or "apartman" in text.lower():
                    result["property_type"] = "residential"
                elif "is yeri" in text.lower() or "dukk" in text.lower() or "ofis" in text.lower() or "ticari" in text.lower():
        ...
        if body_text:
            ...
            result["property_type"] = result.get("property_type") or fields.get("Konut Tipi") or fields.get("İmar Durumu") or fields.get("Türü") or title
            result["date_posted"] = fields.get("İlan Güncelleme Tarihi") or fields.get("İlan Tarihi")
        ...
        # Owner vs agent
        owner_text = await _safe_text(page, ".owner-info, [class*='agent'], [class*='advisor']") or ""
        result["agent_type"] = result.get("agent_type") or infer_agent_type(owner_text)
